...
- (void)viewDidLoad {
    [super viewDidLoad];
    storyTitles = [[NSMutableArray alloc] init];
    [self getTitles];
    ...
    [NSURLConnection sendAsynchronousRequest:request
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
                               json = [NSJSONSerialization JSONObjectWithData:data
                                                                      options:0
                                                                        error:nil];
                               NSLog(@"%@", json[@"posts"][5][@"title"]);
                               for(int i = 0; i < 100; i++){
                                   NSString *title = json[@"posts"][i][@"title"];
                                   NSLog(@"%@", title);
                                   [storyTitles insertObject:title atIndex: i];
                               }
                               NSLog(@"%@", [storyTitles objectAtIndex:5]);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = [storyTitles objectAtIndex:indexPath.row];
